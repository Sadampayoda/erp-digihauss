...
@section('content')
    <div class="flex flex-col  bg-white w-full h-full py-4 lg:py-7 px-1 lg:px-5 shadow rounded-2xl gap-5">
        <div class="flex flex-row justify-between items-center">
            ...
            <a href="{{ route('receipt-invoices.create') }}"
                class="
                    group flex items-center gap-2
        ...
        <x-table :data="$receipt_invoices" :labels="[
            'transaction_number' => 'No. RI',
            'transaction_date' => 'Tgl RI',
            'vendor_name' => 'Vendor',
            'status' => 'Status',
    ...
    <script>
        const onEdit = (id, data) => {
            let url = "{{ route('receipt-invoices.edit', ':id') }}";
        ...
        const onDelete = (id) => {
            Swal.fire({
                ...
                text: 'Data Invoice Pembelian ini akan dihapus permanen!',
                icon: 'warning',
