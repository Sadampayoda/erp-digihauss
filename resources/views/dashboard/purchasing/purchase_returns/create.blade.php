...
@section('content')
    ...
    <div class="flex flex-col lg:flex-row w-full gap-5 overflow-hidden">
        <div class="flex flex-col w-full lg:w-2/3 min-w-0 overflow-hidden gap-3 ">
            ...
            @include('dashboard.purchasing.purchase_returns.partials.general_form')
            <div class="bg-white rounded-xl shadow min-w-0 ">
                ...
                p-4 border-b border-slate-100 mx-3 sm:mx-5 gap-3">
                    ...
                    <button onclick="onTakeRi()" type="button" id="btn-sales-invoices-modal"
                        class="
                ...
                @include('dashboard.purchasing.purchase_returns.partials.items_form')
            </div>
        </div>
        @include('dashboard.purchasing.purchase_returns.partials.invoice')
    </div>
    ...
    <script>
        ...
        const submit = () => {
            setButtonLoading(true, 'purchase-returns-modal-button', 'btn-text-purchase-return');
            ...
            $.ajax({
                ...
                success: function(res) {
                    setButtonLoading(false, 'purchase-returns-modal-button', 'btn-text-purchase-return');
        ...
        const onTakeRi = () => {
            const receiptInvoiceId = document.getElementById('receipt_invoice_id').value;
            if (!receiptInvoiceId) {
                showAlert('Gagal ambil data', 'Pilih data transaksi RI terlebih dahulu', 'errors', true);
                return;
            }
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `/receipt-invoices/${receiptInvoiceId}`,
                        type: 'GET',
                        data: {
                            _token: '{{ csrf_token() }}',
                            _method: 'GET'
                        ...
                        success: function(res) {
                            ...
                            data.items.forEach((item) => {
                                item.receipt_invoice_items_id = item.id
                                item.id = item?.item?.id
                                item.image = item?.item?.image
                                item.name = item.item_name;
                                item.ri_quantity = item.quantity
                                item.quantity = 1
                            ...
                            lockDetailTableColumns(
                                ['sale_price', 'service','purchase_price'], {
                                    mode: 'readonly'
                                }
                            )
                        },
