...
@section('content')
    ...
    <div class="flex flex-col lg:flex-row w-full gap-5 overflow-hidden">
        <div class="flex flex-col w-full lg:w-2/3 min-w-0 overflow-hidden gap-3 ">
            @include('dashboard.purchasing.advance_payments.partials.general_form')
            <div class="bg-white rounded-xl shadow min-w-0 ">
                ...
                @include('dashboard.purchasing.advance_payments.partials.items_form')
            </div>
        </div>
        @include('dashboard.purchasing.advance_payments.partials.invoice')
    </div>
    ...
    <script>
        ...
        const submit = () => {
            setButtonLoading(true, 'advance-payments-modal-button', 'btn-text-advance-payments');
            ...
            $.ajax({
                ...
                success: function(res) {
                    setButtonLoading(false, 'advance-payments-modal-button', 'btn-text-advance-payments');
                    showAlert('Sukses', res.message, 'success', false,
                    "{{ route('advance-payments.index') }}");
                },
                error: function(err) {
                    const message = err.responseJSON.message;
                    setButtonLoading(false, 'advance-payments-modal-button', 'btn-text-advance-payments');
                    showAlert('Gagal Modifikasi Uang Muka', message, 'errors', false);
                    resetErrors();
