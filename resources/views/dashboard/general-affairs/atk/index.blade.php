...
@section('content')
    <div class="flex flex-col  bg-white w-full h-full py-4 lg:py-7 px-1 lg:px-5 shadow rounded-2xl gap-5">
        <div class="flex flex-row justify-between items-center">
            ...
            <a href="{{ route('atk-requests.create') }}"
                class="
                    group flex items-center gap-2
        ...
        <x-table :data="$atks" :labels="[
            'transaction_number' => 'No. ATK',
            'transaction_date' => 'Tgl ATK',
            'employed_name' => 'Karyawan ngajukan',
            'status' => 'Status',
            'grand_total' => 'Total Transaksi',
            'appoved_name' => 'Disetujui',
        ]" onEdit="onEdit" onDelete="onDelete" />
    </div>

    <script>
        const onEdit = (id, data) => {
            let url = "{{ route('atk-requests.edit', ':id') }}";
        ...
        const onDelete = (id) => {
            Swal.fire({
                ...
                text: 'Data ATK ini akan dihapus permanen!',
                icon: 'warning',
