    ...
    <div class="flex flex-col w-full  min-w-0 overflow-hidden gap-3 ">
        <div class="flex flex-col rounded-xl bg-white">
            <div
                ...
                <div>
                    <p class="text-xl font-medium">General Form {{ $bank::$typeDesc[$type] }} </p>
                    <p class="text-sm font-medium text-slate-400">Melakukan transaksi {{ $bank::$typeDesc[$type] }}</p>
                </div>
                <button onclick="submit()" id="bank-modal-button"
                    class="
                            ...
                            cursor-pointer
                        ">
                    <span class="flex flex-row gap-2 text-sm lg:text-base font-medium btn-text-bank">
                        <i data-lucide="save"
            ...
            <form id="generalForm"
                ...
                <div>
                    <x-input-text border_color="border-stone-300" class="rounded-sm p-1 md:p-2" :readonly="true"
                        name="transaction_number" label="No. Transaksi {{ $bank::$typeDesc[$type] }}" />
                </div>
                ...
                <div>
                    <x-input-text border_color="border-stone-300" class="rounded-sm p-1 md:p-2" type="date"
                        name="transaction_date" label="Tanggal {{ $bank::$typeDesc[$type] }}" :required="true" />
                </div>
...
<script>
    const submit = () => {
        setButtonLoading(true, 'bank-modal-button', 'btn-text-bank');
        ...
        $.ajax({
            ...
            success: function(res) {
                setButtonLoading(false, 'bank-modal-button', 'btn-text-bank');
                showAlert('Sukses', res.message, 'success', false,
                    "{{ route('banks.index', ['type' => $type]) }}");
            },
            error: function(err) {
                const message = err.responseJSON.message;
                setButtonLoading(false, 'bank-modal-button', 'btn-text-bank');
                showAlert('Gagal Modifikasi Bank', message, 'errors', false);
                resetErrors();
