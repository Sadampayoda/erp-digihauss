...
@section('content')
    <div class="flex flex-col  bg-white w-full h-full py-4 lg:py-7 px-1 lg:px-5 shadow rounded-2xl gap-5">
        ...
        <div class="flex flex-row justify-between items-center">
            ...
            <a id="btn-open-payment-method-modal"
                class="
                    group flex items-center gap-2
        ...
        <x-table :labels="[
            'code' => 'Kode',
            'name' => 'Nama',
        ]" :data="$brands" onEdit="openEditPaymentMethodModal" onDelete="onDelete" />
    </div>

    @include('dashboard.master.payment_method.partials.model')
    <script>
        function openEditPaymentMethodModal(id, data) {
            document.getElementById('title-modal').innerText = 'Edit Metode Pembayaran';
            const form = document.getElementById('form-modal');

            form.querySelector('[name="code"]').value = data.code;
            form.querySelector('[name="name"]').value = data.name;

            document.getElementById('form-modal').action = `/payment-methods/${id}`;
