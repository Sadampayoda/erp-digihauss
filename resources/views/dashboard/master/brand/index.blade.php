...
@section('content')
    <div class="flex flex-col  bg-white w-full h-full py-4 lg:py-7 px-1 lg:px-5 shadow rounded-2xl gap-5">
        ...
        <x-table :labels="[
            'code' => 'Kode',
            'name' => 'Nama',
        ]" :data="$brands" onEdit="openEditBrandModal" onDelete="onDelete" />
    </div>

    @include('dashboard.master.brand.partials.model')
    <script>
        function openEditBrandModal(id, data) {
            document.getElementById('title-modal').innerText = 'EditBrand';
            const form = document.getElementById('form-modal');

            form.querySelector('[name="code"]').value = data.code;
            form.querySelector('[name="name"]').value = data.name;

            document.getElementById('form-modal').action = `/brands/${id}`;
