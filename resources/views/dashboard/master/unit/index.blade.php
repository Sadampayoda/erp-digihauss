...
@section('content')
    <div class="flex flex-col  bg-white w-full h-full py-4 lg:py-7 px-1 lg:px-5 shadow rounded-2xl gap-5">
        ...
        <x-table :labels="[
            'code' => 'Kode',
            'name' => 'Nama',
        ]" :data="$units" onEdit="openEditUnitModal" onDelete="onDelete" />
    </div>

    @include('dashboard.master.unit.partials.model')
    <script>
        function openEditUnitModal(id, data) {
            document.getElementById('title-modal').innerText = 'Edit Unit';
            const form = document.getElementById('form-modal');

            form.querySelector('[name="code"]').value = data.code;
            form.querySelector('[name="name"]').value = data.name;

            document.getElementById('form-modal').action = `/units/${id}`;
        ...
        const onDelete = (id) => {
            Swal.fire({
                ...
                text: 'Data unit ini akan dihapus permanen!',
                icon: 'warning',
