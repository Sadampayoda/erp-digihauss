...
@section('content')
    <div class="flex flex-col  bg-white w-full h-full py-4 lg:py-7 px-1 lg:px-5 shadow rounded-2xl gap-5">
        ...
        <x-table :data="$closing_days" :labels="[
            'user_name' => 'Nama User',
            'transaction_date' => 'Closing Day',
            'total_sales' => 'Total Sales',
            'cash_expected' => 'Expektasi Kas',
            'total_stock_expected' => 'Ekpektasi Stock',
            'status' => 'Status Closing',
        ]" onEdit="onEdit" titleEdit="Lihat" />
    </div>

    <script>
        function onEdit(id, data) {
            let url = "{{ route('daily-closings.edit', ':id') }}";
